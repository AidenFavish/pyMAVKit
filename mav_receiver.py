...
class Receiver:
    ...
    def wait_for_msg(self, msg: MAVMessage, timeout_seconds:float=-1.0, blocking=True) -> MAVMessage:
        """
        Will wait for msg to occur. Once it does, will return the updated object.
        If blocking will return a FutureMsg.
        """
        if not blocking:
            msg._thread = threading.Thread(target=lambda: self.wait_for_msg(msg), daemon=True)
            msg._thread.start()
            return msg
